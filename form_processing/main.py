...
import streamlit as st
from app.interface.teach import TemplateTeachingInterface
from app.interface.process import ProcessingInterface
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("""
        ### Instructions
        1. Select mode (Process or Create Template)
        2. Upload form(s)
    ...
    try:
        if st.session_state.mode == 'teach':
            teaching_interface = TemplateTeachingInterface()
            teaching_interface.render()
        else:
            processing_interface = ProcessingInterface()
            processing_interface.render()
